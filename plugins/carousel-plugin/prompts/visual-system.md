...
EVERY SLIDE NEEDS 6-10 ELEMENTS:
Premium slides are NOT empty. They have dense, well-organized content:
- Kicker label (contextual, above headline)
- Headline (72-96px, the primary message)
- Subtitle or supporting line
- Data visualization OR key insight container OR metric pair
- Supporting text or description
- Accent element (underline, separator, or badge)

Each element has a clear role. Nothing decorative. Nothing filler. But the slide should feel FULL and ORGANIZED, like a premium magazine spread — not a blank page with 3 words.
...
WHITESPACE:
30-35% of canvas empty. Not 50% (too empty). Not 20% (too crowded). The slide should feel ORGANIZED, not sparse. Every zone has content — header, main, data, footer.

⚠️ NO BACKGROUND SHAPES. NO <svg>/<defs>/<style>. ONLY content elements.

⚠️ SAFE AREA: x:{{SAFE_X_MIN}}-{{SAFE_X_MAX}}, y:73-1252 (ABSOLUTE)
- TOP: y:73-280 is LOGO AREA — keep empty
- BOTTOM: NOTHING below y:{{FOOTER_START}}
